...
def deep_analyze_stock(sym: str, market_regime: dict = None) -> dict:
    ...
    ctx = compute_base(sym, market_regime, tf_substring_al=False)
    if ctx.get("error"):
    ...
    # NOT: Bu motorda özet mesajları orijinalde de sonuç sözlüğünde kullanılmaz
    # (summary = sig['summary']), bu yüzden mesajlar atılır (boş listeye yazılır).
    ...
    composite, rr_ratio, alpha_text, karar = finalize_composite(
        ...
        clamp_100_after_alpha=False, summary=[],
    )
