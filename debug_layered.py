...
def run_test(ticker):
    ...
    try:
        ...
        # IND 1: Auto Trend Lines
        auto_trend_data = []
        if len(df) > 40:
            df['is_min'] = df['Low'] == df['Low'].rolling(window=20, center=True).min()
            p_indices = df[df['is_min']].index.tolist()
            if len(p_indices) >= 2:
        ...
        # IND 9: Volume Profile POC calculation
        ...
        print("SUCCESSFULLY CALCULATED ALL.")
